...
EXPMatcherImplementationBegin(matchValue, (NSUInteger matchIndex, BOOL(^matchBlock)(id value)) )
...
match(^BOOL{
    ...
    if(matchIndex < actualRecorder.valuesSentCount) {
        notRecievedCount = NO;
        BOOL matched = matchBlock(actualRecorder.values[matchIndex]);
        return matched;
    }
...
failureMessageForTo(^NSString *{
    ...
    if(notRecievedCount) {
        NSString *expectedBehaviour = [NSString stringWithFormat:@"to match value at index %@", @(matchIndex)];
    ...
    NSString *expectedBehaviour = [NSString stringWithFormat:@"to match value at index %@", @(matchIndex)];
    NSString *actualBehaviour = [NSString stringWithFormat:@"did not match value at index %@", @(matchIndex)];
    return [[[[[LLReactiveMatchersMessageBuilder message] actual:actualRecorder] expectedBehaviour:expectedBehaviour] actualBehaviour:actualBehaviour] build];
});
...
failureMessageForNotTo(^NSString *{
    ...
    NSString *expectedBehaviour = [NSString stringWithFormat:@"to not match value at index %@", @(matchIndex)];
    NSString *actualBehaviour = [NSString stringWithFormat:@"matched value at index %@", @(matchIndex)];
    return [[[[[LLReactiveMatchersMessageBuilder message] actual:actualRecorder] expectedBehaviour:expectedBehaviour] actualBehaviour:actualBehaviour] build];
});
