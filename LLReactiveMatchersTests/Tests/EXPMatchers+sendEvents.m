...
- (void) test_nonSignalActual {
    ...
    assertFail(test_expect(actual).to.sendEvents(expected), @"Actual (1, 2, 3) is not a Signal");
    assertFail(test_expect(actual).toNot.sendEvents(expected), @"Actual (1, 2, 3) is not a Signal");
}
