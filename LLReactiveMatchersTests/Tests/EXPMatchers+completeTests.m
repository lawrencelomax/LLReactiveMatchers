...
- (void) test_endsInCompletion {
    ...
    assertFail(test_expect(signal).toNot.complete(), @"expected: foo to not complete, got: completed");
}
...
- (void) test_endsInError {
    ...
    assertPass(test_expect(signal).toNot.complete());
    assertFail(test_expect(signal).to.complete(), @"expected: foo to complete, got: error instead of completion");
}
